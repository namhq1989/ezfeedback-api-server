...
func (s server) registerFeedbackRoutes() {
	...
	g.GET("/:id/reply", func(c echo.Context) error {
		var (
			ctx         = c.Get("ctx").(*appcontext.AppContext)
			req         = c.Get("req").(dto.GetFeedbackRepliesRequest)
			performerID = ctx.GetUserID()
			feedbackID  = c.Param("id")
		)

		resp, err := s.app.GetFeedbackReplies(ctx, performerID, feedbackID, req)
		if err != nil {
			return httprespond.R400(c, err, nil)
		}
		...
		return validation.ValidateHTTPPayload[dto.GetFeedbackRepliesRequest](next)
	})
	...
	g.DELETE("/:feedbackId/reply/:replyId", func(c echo.Context) error {
		var (
			ctx         = c.Get("ctx").(*appcontext.AppContext)
			req         = c.Get("req").(dto.DeleteFeedbackReplyRequest)
			performerID = ctx.GetUserID()
			feedbackID  = c.Param("feedbackId")
			replyID     = c.Param("replyId")
		)

		resp, err := s.app.DeleteFeedbackReply(ctx, performerID, feedbackID, replyID, req)
		if err != nil {
			return httprespond.R400(c, err, nil)
		}
		...
		return validation.ValidateHTTPPayload[dto.DeleteFeedbackReplyRequest](next)
	})
}
